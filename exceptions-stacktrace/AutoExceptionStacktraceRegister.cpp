...
namespace ExceptionsStacktrace
{
	...
	LONG WINAPI
		AutoExceptionStacktraceRegister::addStackTraceToException(
		...
		if (isCxxException)
		{
			...
			if (!isSimpleRethrow)
			{
				auto currentExceptionsArguments = std::vector<size_t>(
					...
					reinterpret_cast<size_t*>(ExceptionInfo->ExceptionRecord->ExceptionInformation + ExceptionInfo->ExceptionRecord->NumberParameters));
